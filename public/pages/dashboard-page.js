...
async function loadDashboardStats() {
  try {
    ...
    if (data.success) {
      ...
      if (activeTradesEl) {
        activeTradesEl.textContent = stats.activeTrades;
      }
      
      if (aiAccuracyEl) {
        aiAccuracyEl.textContent = `${stats.aiAccuracy.toFixed(1)}%`;
      }
    }
...
async function loadProfitChart(days) {
  try {
    ...
    const user = JSON.parse(userJson);
    const response = await fetch(`/api/dashboard/profit-chart?userId=${user.id}&days=${days}`);
    const data = await response.json();
...
function renderProfitChart(data, days) {
  ...
  // Генерация полного диапазона дат
  const dates = [];
  const profits = [];
...
function setupPeriodSelector() {
  ...
  periodBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // Загрузка графика с новым периодом
